...
use cap_schema::hierarchy::v1::{BoundingBox, PlatoonSummary, SquadSummary};
...
impl StateAggregator {
    ...
    pub fn aggregate_squad(
        ...
        let aggregated_capabilities = aggregated_capabilities_map
            .into_iter()
            .map(
                |(cap_type, agg_cap)| cap_schema::capability::v1::Capability {
    ...
    fn aggregate_capabilities_from_squads(
        squads: &[SquadSummary],
    ) -> Vec<cap_schema::capability::v1::Capability> {
        use std::collections::HashMap;

        let mut capability_map: HashMap<i32, cap_schema::capability::v1::Capability> =
            HashMap::new();
...
mod tests {
    ...
    use cap_schema::node::v1::HealthStatus as SchemaHealthStatus;
