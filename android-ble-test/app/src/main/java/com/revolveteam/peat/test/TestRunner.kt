...
import com.revolveteam.atak.peat.PeatJni
...
class TestRunner(
    ...
    companion object {
        private const val TAG = "PeatTest"
    ...
    private fun phase1JniInit(): Boolean {
        return try {
            PeatNativeLoader.init(context)
            PeatNativeLoader.loadLibrary("peat_ffi")
            val jniOk = PeatJni.initNatives()
            if (!jniOk) {
            ...
            if (version.isEmpty()) {
                return recordPhase(1, "JNI Init", false, "peatVersion() returned empty")
            }
    ...
    private fun phase2CreateDualNode(): Boolean {
        return try {
            ...
            nodeHandle = PeatJni.createNodeWithConfigJni(
                "FUNCTEST",
                sharedKey,
                storagePath,
                true,  // enableBle
            ...
            val syncOk = PeatJni.startSyncJni(nodeHandle)
            val nodeId = PeatJni.nodeIdJni(nodeHandle)
    ...
    private suspend fun phase5GattSync(): Boolean {
        return try {
            ...
            // Format matches PeatMesh::build_document() wire format
    ...
    private fun phase7MdnsDiscovery(): Boolean {
        return try {
            ...
            for (i in 1..30) {
                Thread.sleep(500)
                peers = PeatJni.peerCountJni(nodeHandle)
                if (peers > 0) {
    ...
    private fun phase8PeerConnect(): Boolean {
        return try {
            ...
            if (!quicAddress.isNullOrEmpty() && !expectedNodeId.isNullOrEmpty()) {
                ...
                try {
                    PeatJni.connectPeerJni(nodeHandle, expectedNodeId, quicAddress)
                } catch (_: Throwable) {}
    ...
    private fun checkForExpectedPeer(expectedNodeId: String?): Boolean {
        val peersJson = PeatJni.connectedPeersJni(nodeHandle)
        return try {
    ...
    private fun phase10SignalBleState(): Boolean {
        return try {
            PeatJni.bleSetStartedJni(nodeHandle, true)

            if (peerNodeIdHex.isNotEmpty()) {
                PeatJni.bleAddPeerJni(nodeHandle, peerNodeIdHex)
            }

            val available = PeatJni.bleIsAvailableJni(nodeHandle)
            val peerCount = PeatJni.blePeerCountJni(nodeHandle)
    ...
    private fun phase11DualTransportVerified(): Boolean {
        return try {
            val irohPeers = PeatJni.peerCountJni(nodeHandle)
    ...
    private fun phase7VerifyDualTransport(): Boolean {
        return try {
            ...
            val bleAvailable = PeatJni.bleIsAvailableJni(nodeHandle)
            val blePeers = PeatJni.blePeerCountJni(nodeHandle)
            val totalPeers = PeatJni.peerCountJni(nodeHandle)
    ...
    private fun phase12HoldForSync(): Boolean {
        return try {
            ...
            for (i in 1..15) {
                Thread.sleep(1000)
                if (i % 5 == 0) {
                    val peers = PeatJni.peerCountJni(nodeHandle)
    ...
    private fun cleanupPhase() {
        ...
        try {
            if (nodeHandle != 0L) {
                try { PeatJni.bleSetStartedJni(nodeHandle, false) } catch (_: Throwable) {}
                PeatJni.freeNodeJni(nodeHandle)
                nodeHandle = 0L
            }
        } catch (e: Throwable) {
