...
use peat_inference::inference::TrackerConfig;
use peat_inference::messages::{Position, Priority, TrackUpdate};
...
use peat_inference::testing::{MetricsCollector, SimulatedC2};
use peat_inference::{
...
/// 2. Advertisement syncs through Peat mesh
...
async fn test_capability_sync_between_nodes() {
    let _ = tracing_subscriber::fmt()
        .with_env_filter("peat_protocol::storage::automerge_sync=debug,peat_protocol::sync=debug")
        .with_test_writer()
        .try_init();
    ...
    let beacon = PeatBeacon::new(beacon_config).unwrap();
    let advertisement = beacon.generate_advertisement().await;
...
async fn test_detection_to_track_flow() {
    ...
    use peat_inference::inference::GroundTruthObject;
    use peat_inference::inference::SimulatedDetectorConfig;
