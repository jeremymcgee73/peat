...
//!
//! The Coordinator is a node in the Peat mesh that aggregates capabilities
//! from multiple teams and presents a formation-level view to C2.
//!
//! # Architecture Note
//!
//! The Peat protocol uses CRDT-based document synchronization (Automerge/Iroh).
...
//! the peat-protocol crate, not this application layer.

use peat_protocol::cell::{AggregatedCapability, CapabilityAggregator};
use peat_protocol::discovery::capability_query::{
    CapabilityQuery, CapabilityQueryEngine, CapabilityStats, QueryMatch,
};
use peat_protocol::models::{CapabilityType, NodeConfig, NodeConfigExt, NodeState, NodeStateExt};
...
///
/// The Coordinator reads team documents from the Peat mesh and presents
/// a unified formation-level view. Network connectivity and document
/// synchronization are handled by the peat-protocol layer.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl Coordinator {
    ...
    ///
    /// In a real deployment, teams would be discovered via the Peat mesh.
    ...
    /// Aggregate capabilities from all teams using peat-protocol
    pub fn get_aggregated_capabilities(
        &self,
    ) -> Result<HashMap<CapabilityType, AggregatedCapability>, peat_protocol::Error> {
    ...
    // ========================================================================
    // Generic Capability Query API (peat-protocol integration)
    // ========================================================================

    /// Query platforms by generic capabilities using peat-protocol's CapabilityQuery
    ///
    /// This provides a generic query interface that works with any capability
    /// type defined in the peat-protocol schema.
    ///
    /// # Example
    ///
    /// ```rust,ignore
    /// use peat_protocol::discovery::CapabilityQuery;
    /// use peat_protocol::models::CapabilityType;
    ///
...
/// Convert a Platform to peat-protocol NodeConfig and NodeState
fn platform_to_node(platform: &Platform) -> (NodeConfig, NodeState) {
    use crate::platform::AuthorityLevel;
    use peat_protocol::models::{
        AuthorityLevel as PeatAuthorityLevel, BindingType, HumanMachinePair, HumanMachinePairExt,
        Operator, OperatorExt, OperatorRank,
    };
    use peat_protocol::traits::Phase;

    let caps = platform.get_capabilities();

    fn convert_authority(auth: AuthorityLevel) -> PeatAuthorityLevel {
        match auth {
            AuthorityLevel::Observer => PeatAuthorityLevel::Observer,
            AuthorityLevel::Advisor => PeatAuthorityLevel::Advisor,
            AuthorityLevel::Supervisor => PeatAuthorityLevel::Supervisor,
            AuthorityLevel::Commander => PeatAuthorityLevel::Commander,
        }
    }
...
mod tests {
    ...
    // ========================================================================
    // Generic Capability Query Tests (peat-protocol integration)
    // ========================================================================
