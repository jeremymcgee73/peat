...
/// Instance owner callback run after a UDP socket is created and configured,
/// but before it is bound to its local address.
///
/// Mobile runtimes use this narrow seam to associate only sockets owned by
/// this crate with an OS-managed network. Returning an error fails the bind;
/// there is no unmarked fallback. The hook is also run on every rebind.
pub trait UdpSocketBindHook: Send + Sync + 'static {
    fn before_bind(&self, requested_addr: SocketAddr, socket: &socket2::Socket) -> io::Result<()>;
}

impl<F> UdpSocketBindHook for F
where
    F: Fn(SocketAddr, &socket2::Socket) -> io::Result<()> + Send + Sync + 'static,
{
    fn before_bind(&self, requested_addr: SocketAddr, socket: &socket2::Socket) -> io::Result<()> {
        self(requested_addr, socket)
    }
}

struct InstalledUdpSocketBindHook {
    id: u64,
    callback: Arc<dyn UdpSocketBindHook>,
}

static UDP_SOCKET_BIND_HOOK: OnceLock<RwLock<Option<InstalledUdpSocketBindHook>>> = OnceLock::new();
static NEXT_UDP_SOCKET_BIND_HOOK_ID: AtomicU64 = AtomicU64::new(1);

/// Exclusive lease for the process-local UDP bind hook.
///
/// The lease prevents unrelated endpoint owners from silently replacing each
/// other's socket policy. Dropping it removes only the hook installed by this
/// lease, so stale cleanup cannot clear a newer owner.
#[derive(Debug)]
pub struct UdpSocketBindHookGuard {
    id: u64,
}

impl Drop for UdpSocketBindHookGuard {
    fn drop(&mut self) {
        let slot = UDP_SOCKET_BIND_HOOK.get_or_init(|| RwLock::new(None));
        let mut installed = slot.write().unwrap_or_else(|error| error.into_inner());
        if installed.as_ref().is_some_and(|hook| hook.id == self.id) {
            *installed = None;
        }
    }
}

/// Install the exclusive socket-owner hook and return its lifetime lease.
///
/// A second live owner is rejected instead of replacing the active policy.
pub fn install_udp_socket_bind_hook(
    callback: Arc<dyn UdpSocketBindHook>,
) -> io::Result<UdpSocketBindHookGuard> {
    let slot = UDP_SOCKET_BIND_HOOK.get_or_init(|| RwLock::new(None));
    let mut installed = slot.write().unwrap_or_else(|error| error.into_inner());
    if installed.is_some() {
        return Err(io::Error::new(
            io::ErrorKind::AlreadyExists,
            "a UDP socket bind hook is already installed",
        ));
    }
    let id = NEXT_UDP_SOCKET_BIND_HOOK_ID.fetch_add(1, Ordering::Relaxed);
    *installed = Some(InstalledUdpSocketBindHook { id, callback });
    Ok(UdpSocketBindHookGuard { id })
}

fn run_udp_socket_bind_hook(addr: SocketAddr, socket: &socket2::Socket) -> io::Result<()> {
    let slot = UDP_SOCKET_BIND_HOOK.get_or_init(|| RwLock::new(None));
    let callback = slot
        .read()
        .unwrap_or_else(|error| error.into_inner())
        .as_ref()
        .map(|installed| Arc::clone(&installed.callback));
    if let Some(callback) = callback {
        callback.before_bind(addr, socket)?;
    }
    Ok(())
}
...
impl SocketState {
    ...
    fn bind(addr: SocketAddr) -> io::Result<Self> {
        ...
        run_udp_socket_bind_hook(addr, &socket)?;
...
mod tests {
    ...
    #[test]
    fn bind_hook_is_exclusive_and_runs_on_bind_and_rebind() -> TestResult {
        let calls = Arc::new(AtomicUsize::new(0));
        let observed = Arc::clone(&calls);
        let guard = install_udp_socket_bind_hook(Arc::new(
            move |_addr: SocketAddr, _socket: &socket2::Socket| {
        ...
        assert!(
            install_udp_socket_bind_hook(Arc::new(
                |_addr: SocketAddr, _socket: &socket2::Socket| Ok(())
            ))
            .is_err()
        );

        let before_bind = calls.load(Ordering::Relaxed);
        let socket = UdpSocket::bind_local_v4(0)?;
        let after_bind = calls.load(Ordering::Relaxed);
        assert!(after_bind > before_bind);
        socket.rebind()?;
        assert!(calls.load(Ordering::Relaxed) > after_bind);

        drop(socket);
        drop(guard);
        let replacement = install_udp_socket_bind_hook(Arc::new(
            |_addr: SocketAddr, _socket: &socket2::Socket| Ok(()),
        ))?;
        drop(replacement);
        Ok(())
