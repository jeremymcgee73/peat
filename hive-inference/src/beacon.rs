//! HIVE Beacon Client
//!
//! Registers the edge device as a sensor platform with the HIVE network,
//! advertising camera and AI model capabilities with real hardware specs.
//!
//! ## Usage
//!
//! ```rust,ignore
//! use hive_inference::beacon::{HiveBeacon, BeaconConfig, CameraSpec, ModelSpec};
//!
//! // Create beacon with auto-detected hardware
//! let beacon = HiveBeacon::new(BeaconConfig::auto_detect()?)?;
//!
...
/// HIVE Beacon - registers edge device with the HIVE network
...
mod tests {
    ...
    async fn test_beacon_advertisement() {
        ...
        let beacon = HiveBeacon::new(config).unwrap();
