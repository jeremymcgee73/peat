...
//! - HIVE blob references (content-addressed storage via iroh-blobs)
//!
//! ## Features
//!
...
//!
//! ```rust,ignore
//! use hive_inference::orchestration::fetcher::{ModelFetcher, FetchConfig};
//!
...
impl ModelFetcher {
    ...
    pub async fn fetch(
        ...
        if source.starts_with("hive://blobs/") {
    ...
    /// Fetch a model from a HIVE blob reference
    ///
    /// Blob references use content-addressed storage via iroh-blobs.
    /// Format: `hive://blobs/sha256:<hash>`
...
mod tests {
    ...
    async fn test_fetch_blob_stub() {
        ...
        let fetcher = ModelFetcher::with_defaults();
        let result = fetcher
            .fetch_blob("hive://blobs/sha256:abc123def456", output_dir)
            .await
            .unwrap();
    ...
    async fn test_fetch_auto_detect() {
        ...
        let result = fetcher
            .fetch("hive://blobs/sha256:abc123", None, output_dir)
            .await;
        assert!(result.is_ok());
