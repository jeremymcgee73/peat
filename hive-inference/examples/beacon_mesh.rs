//! HIVE Beacon Mesh Integration Example
//!
//! Demonstrates connecting an edge device to the HIVE mesh network
...
//!   export HIVE_SECRET_KEY="<base64-secret>"
...
//!   --formation <id>      Formation ID (default: from HIVE_APP_ID env)
//!   --platform-id <id>    Platform identifier (default: auto-generated)

use hive_inference::beacon::{BeaconConfig, CameraSpec, HiveBeacon, ModelSpec};
use hive_inference::inference::JetsonInfo;
use hive_inference::messages::{ModelPerformance, OperationalStatus};
use hive_inference::sync::{collections, HiveSyncClient, SyncConfig};
...
async fn main() -> anyhow::Result<()> {
    ...
    if args.iter().any(|a| a == "--generate-secret") {
        ...
        println!();
        println!("Usage:");
        println!("  export HIVE_APP_ID=\"my-formation\"");
        println!("  export HIVE_SECRET_KEY=\"{}\"", secret);
        println!();
    ...
    let formation_id = args
        ...
        .and_then(|i| args.get(i + 1))
        .cloned()
        .or_else(|| std::env::var("HIVE_APP_ID").ok())
    ...
    let secret_key = std::env::var("HIVE_SECRET_KEY").ok();

    println!("HIVE Beacon Mesh Integration");
    println!("=============================");
    println!();
    ...
    let persistence_dir = PathBuf::from(format!("/tmp/hive-beacon-{}", platform_id));
    ...
    let shared_key = secret_key.unwrap_or_else(|| {
        ...
        println!("  (Set HIVE_SECRET_KEY to share with other nodes)");
        key
    });
    ...
    if let Some(ref info) = jetson_info {
        beacon_config = beacon_config
            ...
            .with_compute(hive_inference::beacon::ComputeSpec::from_jetson(info));
    }

    let beacon = HiveBeacon::new(beacon_config)?;
    ...
    let doc = hive_protocol::sync::types::Document::with_id(&platform_id, fields);
    let doc_id = backend
        .document_store()
    ...
    let all_docs = backend
        ...
        .query(
            ...
            &hive_protocol::sync::types::Query::All,
        )
        .await?;
