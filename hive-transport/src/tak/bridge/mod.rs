//! HIVE-TAK Bridge
//!
//! Bidirectional bridge between HIVE mesh network and TAK ecosystem.
...
/// HIVE message that can be bridged to TAK
#[derive(Debug, Clone)]
...
impl HiveMessage {
    ...
    pub fn source_node(&self) -> &str {
        match self {
            HiveMessage::Track(t) => &t.source_platform,
            HiveMessage::Capability(c) => &c.platform_id,
            HiveMessage::Handoff(h) => &h.source_cell,
            HiveMessage::FormationSummary(f) => &f.formation_id,
        }
    }
    ...
    pub fn echelon(&self) -> EchelonLevel {
        match self {
            ...
            HiveMessage::Capability(_) => EchelonLevel::Platform,
            HiveMessage::Handoff(_) => EchelonLevel::Cell,
            HiveMessage::FormationSummary(_) => EchelonLevel::Formation,
        }
    }
    ...
    pub fn position(&self) -> Option<(f64, f64)> {
        match self {
            HiveMessage::Track(t) => Some((t.position.lat, t.position.lon)),
            HiveMessage::Capability(c) => Some((c.position.lat, c.position.lon)),
            _ => None,
        }
    }
}
...
impl<T: TakTransport> HiveTakBridge<T> {
    ...
    /// Encode a HIVE message to CoT with priority
    fn encode_message(&self, message: &HiveMessage) -> Result<(CotEvent, Priority), TakError> {
        let (event, priority) = match message {
            HiveMessage::Track(track) => {
                let event = self
                    .encoder
            ...
            HiveMessage::FormationSummary(summary) => {
                let event = self
                    .encoder
...
mod tests {
    ...
    #[test]
    fn test_hive_message_echelon() {
        use hive_protocol::cot::Position;
        ...
        let msg = HiveMessage::Track(track);
