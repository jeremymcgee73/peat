//! Message filtering for HIVE-TAK bridge
...
impl BridgeFilter {
    ...
    pub fn should_publish(&self, message: &HiveMessage) -> FilterDecision {
        ...
        if let AggregationPolicy::TracksOnly = &self.aggregation_policy {
            match message {
                ...
                HiveMessage::Capability(_) | HiveMessage::FormationSummary(_) => {
                    return FilterDecision::Drop(
                        "TracksOnly: filtering non-track message".to_string(),
                    );
                }
                HiveMessage::Handoff(_) => {
