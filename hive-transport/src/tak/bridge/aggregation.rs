//! Message aggregation for HIVE-TAK bridge

use std::collections::HashMap;
use std::sync::RwLock;

use super::config::AggregationPolicy;
use super::HiveMessage;
...
pub struct Aggregator {
    ...
    pending: RwLock<HashMap<String, HiveMessage>>,
}
...
impl Aggregator {
    ...
    pub fn add(&self, message: HiveMessage) {
    ...
    pub fn flush(&self) -> Vec<HiveMessage> {
        let mut pending = self.pending.write().unwrap();
...
mod tests {
    use super::*;
    use hive_protocol::cot::{Position, TrackUpdate};

    fn make_track(id: &str, platform: &str) -> HiveMessage {
        HiveMessage::Track(TrackUpdate {
    ...
    fn test_aggregator_replaces_same_source() {
        ...
        if let HiveMessage::Track(track) = &flushed[0] {
