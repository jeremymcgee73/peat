...
HTTP/REST API transport layer for HIVE Protocol external integration.

## Overview

`hive-transport` provides external API access to CAP node state, enabling C2 dashboards, legacy systems, and monitoring tools to query the CAP mesh network via standard HTTP/REST endpoints.
...
- **JSON responses**: Uses hive-schema protobuf → JSON encoding
...
- [hive-schema](../hive-schema/README.md) - Protocol Buffer message definitions
- [hive-protocol](../hive-protocol/README.md) - Core HIVE Protocol implementation
