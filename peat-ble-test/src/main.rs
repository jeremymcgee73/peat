...
//! ```bash
//! cargo run -p peat-ble-test -- --node-id DEADBEEF mesh
//! ```
//!
//! ## Platform Support
//!
...
#[cfg(not(any(target_os = "macos", target_os = "linux")))]
use peat_btle::StubAdapter;
use peat_btle::{BleConfig, BluetoothLETransport};
use peat_protocol::transport::{
    MessageRequirements, NodeId, PeatBleTransport, TransportCapabilities, TransportInstance,
...
#[cfg(target_os = "macos")]
async fn create_transport(node_id: u32) -> Result<PeatBleTransport<CoreBluetoothAdapter>> {
    use peat_btle::platform::BleAdapter;

    info!("Using CoreBluetoothAdapter for macOS");

    let config = BleConfig::peat_lite(peat_btle::NodeId::new(node_id));
    let mut adapter =
...
#[cfg(target_os = "linux")]
async fn create_transport(node_id: u32) -> Result<PeatBleTransport<BluerAdapter>> {
    use peat_btle::platform::BleAdapter;
...
#[cfg(not(any(target_os = "macos", target_os = "linux")))]
async fn create_transport(node_id: u32) -> Result<PeatBleTransport<StubAdapter>> {
    use peat_btle::platform::BleAdapter;
...
async fn run_scan(manager: &TransportManager, duration: Duration) -> Result<()> {
    info!("Scanning for Peat nodes for {:?}...", duration);
