...
if ! docker image inspect cap-sim-node:latest &> /dev/null; then
    echo -e "${RED}✗ cap-sim-node:latest image not found${NC}"
    echo "  Build it with: docker build -f cap-sim/Dockerfile -t cap-sim-node:latest ."
    exit 1
fi
