#!/bin/bash
# Test all topology modes with baseline (raw Ditto without CAP) under bandwidth constraints
#
# This script compares raw Ditto performance (shadow_poc) vs CAP Protocol overhead
# by running the same tests with USE_BASELINE=true
#
# Bandwidth levels:
...
echo "======================================"
echo "E8 Bandwidth Baseline Testing"
echo "(Raw Ditto - No CAP Protocol)"
echo "======================================"
echo ""
...
RESULTS_BASE_DIR="test-results-baseline-$TIMESTAMP"
...
# Function to create baseline topology (inject USE_BASELINE=true)
create_baseline_topology() {
    local source_topology=$1
    local baseline_topology=$2

    # Use sed to add USE_BASELINE: "true" after each NODE_ID line
    sed '/NODE_ID:/a\        USE_BASELINE: "true"' "$source_topology" > "$baseline_topology"
}
...
for bw_name in "100mbps" "10mbps" "1mbps" "256kbps"; do
    ...
    for mode_config in "${MODES[@]}"; do
        ...
        echo "--------------------------------------"
        echo "Testing: $mode_name at $bw_name (BASELINE)"
        echo "--------------------------------------"

        start_time=$(date +%s)

        # Create baseline topology with USE_BASELINE=true
        baseline_topology="/tmp/baseline_${mode_name}.yaml"
        echo "[$(date +%T)] Creating baseline topology..."
        create_baseline_topology "$topology_file" "$baseline_topology"

        # Deploy topology
        echo "[$(date +%T)] Deploying baseline topology..."
        containerlab destroy --all --cleanup > /dev/null 2>&1 || true
        sleep 2

        deploy_start=$(date +%s)
        containerlab deploy -t "$baseline_topology" > "$BW_RESULTS_DIR/${mode_name}_deploy.log" 2>&1
        deploy_time=$(($(date +%s) - deploy_start))
        echo "[$(date +%T)] Deployment complete in ${deploy_time}s"

        # Extract lab name from topology file
        lab_name=$(grep "name:" "$baseline_topology" | head -1 | awk '{print $2}')
...
Mode: $mode_name (BASELINE - Raw Ditto)
...
# E8 Baseline Testing - Comprehensive Results
## Raw Ditto Performance (No CAP Protocol)

**Test Date:** $(date)

## Test Overview

Tested three topology modes with bandwidth constraints using **baseline Ditto** (shadow_poc):
- 100Mbps (unconstrained baseline)
- 10Mbps (typical WiFi/LTE)
- 1Mbps (constrained wireless)
- 256Kbps (tactical radio)

**IMPORTANT:** These tests use `shadow_poc` binary (raw Ditto SDK) WITHOUT CAP Protocol overhead.
This provides the performance baseline for comparison with CAP Protocol results.
...
These results show **raw Ditto SDK performance** without any CAP Protocol overhead:
- **Convergence time**: Time for Ditto CRDT sync across all nodes
- **Per-update latency**: Individual message transmission time
- **Consistency**: System behavior across topology modes

### Comparison with CAP Protocol

Compare these baseline results with CAP Protocol results to measure:
1. **CAP Protocol Overhead**: Difference in convergence time and latency
2. **Capability Check Cost**: Impact of authorization checks
3. **Differential Updates**: Bandwidth savings from selective replication

### Expected Differences

CAP Protocol adds overhead for:
- Capability verification on each document access
- Differential update computation and filtering
- Additional metadata for fine-grained access control

However, CAP should show benefits in:
- Reduced bandwidth (only authorized data synced)
- Better scalability with large datasets
- Fine-grained access control without full mesh trust
