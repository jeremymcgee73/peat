# CAP Protocol Network Simulation with ContainerLab

This directory contains ContainerLab-based network simulation infrastructure for testing CAP Protocol under realistic network conditions.
...
- **Rust 1.86+**: For building cap-sim-node image
...
- Ditto SDK (v4.11.5)
- CAP Protocol simulation node binary
...
```bash
# Copy example file
cp cap-sim/.env.example cap-sim/.env
...
```
cap-sim/
├── Dockerfile              # cap-sim-node image definition
...
docker build --no-cache -f cap-sim/Dockerfile -t cap-sim-node:latest .
```
