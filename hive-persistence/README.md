...
`hive-persistence` provides a backend-agnostic interface for persisting and querying HIVE protocol data, enabling external systems to access CAP state without coupling to specific storage implementations.
...
hive-persistence = { path = "../hive-persistence", features = ["external-api"] }
```
...
The current production implementation wraps the existing `DataSyncBackend` from `hive-protocol`.
...
`hive-persistence` is part of the larger HIVE Protocol ecosystem:

- **hive-schema**: Protobuf message definitions (stores these messages)
- **hive-protocol**: Core protocol logic (uses this for state storage)
- **hive-transport**: HTTP/gRPC transports (complementary external access)
...
- [HIVE Protocol Documentation](../README.md)
