...
use peat_protocol::command::{CommandCoordinator, CommandStorage, ObserverHandle};
use peat_protocol::storage::{ditto_store::DittoConfig, DittoStore};
...
impl CommandStorage for MockCommandStorage {
    ...
    async fn get_command(
        &self,
        _command_id: &str,
    ) -> peat_protocol::Result<Option<HierarchicalCommand>> {
        Ok(None)
    }
    ...
    async fn query_commands_by_target(
        &self,
        _target_id: &str,
    ) -> peat_protocol::Result<Vec<HierarchicalCommand>> {
        Ok(Vec::new())
    }

    async fn delete_command(&self, _command_id: &str) -> peat_protocol::Result<()> {
        Ok(())
    }
    ...
    async fn get_acknowledgments(
        &self,
        _command_id: &str,
    ) -> peat_protocol::Result<Vec<CommandAcknowledgment>> {
        Ok(Vec::new())
    }

    async fn update_command_status(&self, _status: &CommandStatus) -> peat_protocol::Result<()> {
        Ok(())
    }
    ...
    async fn get_command_status(
        &self,
        _command_id: &str,
    ) -> peat_protocol::Result<Option<CommandStatus>> {
        Ok(None)
    }
    ...
    async fn observe_commands(
        ...
        _callback: Box<
            dyn Fn(
                ...
                + Sync,
        >,
    ) -> peat_protocol::Result<ObserverHandle> {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Load credentials from environment via PeatCredentials (supports PEAT_* with DITTO_* fallback)
    dotenvy::dotenv().ok();

    let credentials = peat_protocol::credentials::PeatCredentials::from_env()
        .map_err(|e| format!("Credentials error: {}. Please set PEAT_APP_ID/PEAT_SECRET_KEY or DITTO_APP_ID/DITTO_SHARED_KEY in .env", e))?;
