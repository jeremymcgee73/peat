...
//!
//! This module implements automatic peer discovery for Peat Protocol nodes using
//! multiple strategies:
//!
...
///
/// Advertises this node's presence on the local network and discovers other Peat nodes.
/// Uses the service type `_peat-node._tcp.local` for discovery.
///
/// # Service Advertisement Format
///
/// Each node advertises via mDNS with:
/// - **Service Type**: `_peat-node._tcp.local`
/// - **Instance Name**: `<node-name>._peat-node._tcp.local`
/// - **TXT Records**:
...
impl MdnsDiscovery {
    /// Service type for Peat Protocol nodes on mDNS
...
impl DiscoveryStrategy for MdnsDiscovery {
    async fn start(&mut self) -> anyhow::Result<()> {
        ...
        // Browse for other Peat nodes using the same daemon
        let receiver = mdns
