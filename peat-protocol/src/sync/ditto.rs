...
impl DataSyncBackend for DittoBackend {
    async fn initialize(&self, config: BackendConfig) -> anyhow::Result<()> {
        // Get offline_token from extra config or environment (via PeatCredentials)
        let offline_token = config.extra.get("offline_token").cloned()
            .or_else(|| {
                crate::credentials::PeatCredentials::try_from_env()
                    .and_then(|c| c.offline_token().map(|s| s.to_string()))
            })
            .ok_or_else(|| {
                Error::config_error(
                    "offline_token required for Ditto backend (set PEAT_OFFLINE_TOKEN or DITTO_OFFLINE_TOKEN)",
...
mod tests {
    ...
    ///
    /// Uses PeatCredentials to load credentials (supports both PEAT_* and DITTO_* env vars)
    fn create_test_config() -> BackendConfig {
        // Load environment for credentials
        dotenvy::dotenv().ok();

        // Use PeatCredentials to load app_id and shared_key (with DITTO_* fallback)
        let (app_id, shared_key) = if let Ok(creds) = PeatCredentials::from_env() {
            (
    ...
    async fn test_is_ready() {
        ...
        // PeatCredentials checks for PEAT_OFFLINE_TOKEN with fallback to DITTO_OFFLINE_TOKEN
        if let Ok(creds) = PeatCredentials::from_env() {
    ...
    async fn test_trait_two_node_sync() {
        ...
        // Skip test if credentials not available (checks PEAT_* with DITTO_* fallback)
        let credentials = match PeatCredentials::from_env() {
            Ok(creds) if creds.has_secret_key() => creds,
            _ => {
                eprintln!("Skipping test: credentials not available (need PEAT_APP_ID/PEAT_SECRET_KEY or DITTO_APP_ID/DITTO_SHARED_KEY)");
                return;
            }
        };
