...
`peat-discovery` provides pluggable discovery strategies for finding and tracking peers in a distributed mesh network. This is **Layer 1** of the P2P Mesh Intelligence architecture (ADR-017).
...
```rust
use peat_discovery::{MdnsDiscovery, DiscoveryStrategy};
...
```rust
use peat_discovery::{StaticDiscovery, DiscoveryStrategy};
use std::path::Path;
...
```
┌─────────────────────────────────────────┐
│     Layer 3: Data Flow Control          │  (peat-mesh - future)
│   Routing, Aggregation, Priority        │
└─────────────────┬───────────────────────┘
                  │
┌─────────────────▼───────────────────────┐
│  Layer 2: Mesh Topology Management      │  (peat-mesh - future)
...
```bash
cargo test -p peat-discovery
```

Run with logging:

```bash
RUST_LOG=debug cargo test -p peat-discovery -- --nocapture
```
...
- [EPIC 2: P2P Mesh Intelligence Layer](https://github.com/defenseunicorns/peat/issues/105)
