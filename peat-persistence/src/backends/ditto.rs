...
use peat_protocol::sync::{DataSyncBackend, Document as SyncDocument, Query as SyncQuery};
        ...
        tokio::spawn(async move {
            ...
            while let Some(event) = receiver.recv().await {
                let change_event = match event {
                    peat_protocol::sync::ChangeEvent::Initial { documents } => {
                        ChangeEvent::Initial {
                            count: documents.len(),
                        }
                    }
                    peat_protocol::sync::ChangeEvent::Updated { document, .. } => {
                    ...
                    peat_protocol::sync::ChangeEvent::Removed { doc_id, .. } => {
                        ChangeEvent::Delete {
