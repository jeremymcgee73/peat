...
/**
 * Manages HIVE platform markers on the ATAK map.
 *
 ...
 */
class HivePlatformOverlay(private val mapView: MapView) {

    companion object {
        private const val TAG = "HivePlatformOverlay"
    ...
    private fun initMapGroup() {
        ...
        // Find or create HIVE Platforms group
        mapGroup = rootGroup.findMapGroup(GROUP_NAME)
        if (mapGroup == null) {
            mapGroup = rootGroup.addGroup(GROUP_NAME)
            Log.i(TAG, "Created HIVE Platforms map group")
        } else {
            Log.d(TAG, "Found existing HIVE Platforms map group")
        }
    }
     ...
     */
    fun updatePlatforms(platforms: List<HivePlatform>, cells: List<HiveCell>) {
     ...
     */
    private fun createMarker(platform: HivePlatform, cellNameMap: Map<String, String>) {
        ...
        try {
            ...
            // Add metadata
            marker.setMetaString("hivePlatformId", platform.id)
     ...
     */
    private fun updateMarker(marker: Marker, platform: HivePlatform, cellNameMap: Map<String, String>) {
        try {
            val point = GeoPoint(platform.lat, platform.lon, platform.hae ?: 0.0)
            marker.point = point
     ...
     */
    private fun buildTitle(platform: HivePlatform, cellName: String?): String {
    ...
    private fun applyMarkerStyle(marker: Marker, platform: HivePlatform) {
        ...
        val finalColor = when (platform.status) {
            HivePlatform.Status.DEGRADED,
            HivePlatform.Status.LOST_COMMS,
            HivePlatform.Status.LOW_POWER -> dimColor(color)
            HivePlatform.Status.OFFLINE -> Color.DKGRAY
            else -> color
        }
    ...
    fun dispose() {
        ...
        mapGroup = null
        Log.i(TAG, "HivePlatformOverlay disposed")
    }
}
