 ...
 */
class PeatTrackOverlay(private val mapView: MapView) {
     ...
     */
    fun updateTracks(tracks: List<PeatTrack>) {
     ...
     */
    private fun createMarker(track: PeatTrack) {
        ...
        try {
            ...
            marker.setMetaString("peatTackId", track.id)
     ...
     */
    private fun updateMarker(marker: Marker, track: PeatTrack) {
        try {
            val point = GeoPoint(track.lat, track.lon, track.hae ?: 0.0)
            marker.point = point
     ...
     */
    private fun applyMarkerStyle(marker: Marker, track: PeatTrack) {
        ...
        val iconUri = when (track.category) {
            PeatTrack.Category.PERSON -> "asset://icons/person.png"
            PeatTrack.Category.VEHICLE -> "asset://icons/vehicle.png"
            PeatTrack.Category.AIRCRAFT -> "asset://icons/aircraft.png"
            PeatTrack.Category.VESSEL -> "asset://icons/vessel.png"
    ...
    fun dispose() {
        ...
        mapGroup = null
        Log.i(TAG, "PeatTrackOverlay disposed")
    }
}
