...
- Day 1: Create Ditto-based `peat-sim-node` container
...
soldier1.cmd('/path/to/peat-sim-node --node-id soldier1 &')
soldier2.cmd('/path/to/peat-sim-node --node-id soldier2 &')
...
- Days 2-3: Build scenario generator, integrate peat-sim-node
...
- Days 2-3: Create scenario scripts, integrate peat-sim-node
...
2. Create simple `peat-sim-node` Dockerfile with Ditto
...
2. Create `peat-sim-node` Docker image with Ditto
