...
use hive_schema::event::v1::{EventPriority, HiveEvent};
...
pub struct PriorityEventQueue {
    ...
    queues: [VecDeque<HiveEvent>; PRIORITY_LEVELS],
}
...
impl PriorityEventQueue {
    ...
    pub fn push(&mut self, event: HiveEvent) {
    ...
    pub fn pop(&mut self) -> Option<HiveEvent> {
    ...
    pub fn pop_critical(&mut self) -> Vec<HiveEvent> {
    ...
    pub fn pop_weighted(&mut self, max_events: usize) -> Vec<HiveEvent> {
        if max_events == 0 {
            return Vec::new();
        }
    ...
    fn get_priority(&self, event: &HiveEvent) -> EventPriority {
        event
            .routing
            .as_ref()
    ...
    fn pop_from_level(&mut self, level: usize, count: usize, result: &mut Vec<HiveEvent>) -> usize {
        let mut popped = 0;
        while popped < count {
...
mod tests {
    use super::*;
    use hive_schema::event::v1::AggregationPolicy;

    fn make_event(id: &str, priority: EventPriority) -> HiveEvent {
        HiveEvent {
            ...
            event_class: hive_schema::event::v1::EventClass::Product as i32,
            event_type: "test".to_string(),
            routing: Some(AggregationPolicy {
                propagation: hive_schema::event::v1::PropagationMode::PropagationFull as i32,
