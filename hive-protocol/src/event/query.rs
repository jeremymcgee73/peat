...
use hive_schema::event::v1::{
    EventClass, EventFilters, EventQuery, EventQueryResponse, HiveEvent, QueryScope,
};
...
pub struct InMemoryEventStore {
    events: RwLock<HashMap<String, HiveEvent>>,
}
...
impl EventQueryHandler {
    ...
    pub fn store_event(&self, event: HiveEvent) {
    ...
    pub fn handle_query(&self, query: &EventQuery) -> QueryResult {
        ...
        if let Some(scope) = scope {
            if let Some(target) = &scope.target {
                match target {
                    hive_schema::event::v1::query_scope::Target::NodeId(node_id) => {
                    ...
                    hive_schema::event::v1::query_scope::Target::FormationId(formation_id) => {
                    ...
                    hive_schema::event::v1::query_scope::Target::Subordinates(_) => {
    ...
    pub fn merge_responses(
        ...
        let mut all_events: Vec<HiveEvent> = responses
            .into_iter()
    ...
    pub fn create_node_query(
        ...
        EventQuery {
            ...
            scope: Some(QueryScope {
                target: Some(hive_schema::event::v1::query_scope::Target::NodeId(
                    node_id.to_string(),
                )),
            }),
            filters,
    ...
    pub fn create_formation_query(
        ...
        EventQuery {
            ...
            requester_id: requester_id.to_string(),
            scope: Some(QueryScope {
                target: Some(hive_schema::event::v1::query_scope::Target::FormationId(
    ...
    pub fn create_subordinates_query(
        ...
        EventQuery {
            ...
            requester_id: requester_id.to_string(),
            scope: Some(QueryScope {
                target: Some(hive_schema::event::v1::query_scope::Target::Subordinates(
                    true,
                )),
            }),
            filters,
...
mod tests {
    ...
    use hive_schema::event::v1::{AggregationPolicy, EventPriority, PropagationMode};

    fn make_event(id: &str, event_type: &str, timestamp_seconds: u64) -> HiveEvent {
        HiveEvent {
