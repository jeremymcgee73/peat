//! HIVE custom detail extension schema for CoT
//!
//! Implements the `<_hive_>` XML extension defined in ADR-028 for preserving
//! HIVE-specific semantics in CoT messages.
...
/// HIVE version for the extension schema
pub const HIVE_EXTENSION_VERSION: &str = "1.0";

/// HIVE custom detail extension
///
/// Contains HIVE-specific metadata that doesn't map to standard CoT fields.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct HiveExtension {
    /// Source attribution
    pub source: Option<HiveSource>,
    /// Confidence information
    pub confidence: Option<HiveConfidence>,
    /// Hierarchy membership
    pub hierarchy: Option<HiveHierarchy>,
    /// Custom attributes
    pub attributes: HashMap<String, HiveAttribute>,
    /// Operational status
    pub status: Option<HiveStatus>,
    /// Capability information (for capability advertisements)
    pub capabilities: Vec<HiveCapability>,
    /// Handoff information (for handoff messages)
    pub handoff: Option<HiveHandoff>,
    /// Classification information
    pub classification: Option<HiveClassification>,
}

impl HiveExtension {
    /// Create a new empty HIVE extension
    pub fn new() -> Self {
        Self::default()
    }

    /// Set source attribution
    pub fn with_source(mut self, source: HiveSource) -> Self {
    ...
    pub fn with_confidence(mut self, value: f64, threshold: Option<f64>) -> Self {
        self.confidence = Some(HiveConfidence { value, threshold });
        self
    }

    /// Set hierarchy membership
    pub fn with_hierarchy(mut self, hierarchy: HiveHierarchy) -> Self {
    ...
    /// Set status
    pub fn with_status(mut self, status: HiveStatus) -> Self {
        self.status = Some(status);
        self
    }

    /// Add a capability
    pub fn with_capability(mut self, capability: HiveCapability) -> Self {
        self.capabilities.push(capability);
        self
    }

    /// Set handoff information
    pub fn with_handoff(mut self, handoff: HiveHandoff) -> Self {
    ...
    pub fn with_classification(mut self, level: &str, caveat: Option<&str>) -> Self {
        self.classification = Some(HiveClassification {
    ...
    pub fn write_xml(&self, writer: &mut Writer<Cursor<Vec<u8>>>) -> Result<(), CotError> {
        let mut hive_elem = BytesStart::new("_hive_");
        hive_elem.push_attribute(("version", HIVE_EXTENSION_VERSION));

        writer
            .write_event(Event::Start(hive_elem))
        ...
        writer
            .write_event(Event::End(BytesEnd::new("_hive_")))
            .map_err(|e| CotError::XmlWrite(e.to_string()))?;
...
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
pub struct HiveHierarchy {
    /// Cell membership
    pub cell: Option<HiveCellMembership>,
...
impl HiveHierarchy {
    ...
    pub fn with_cell(mut self, id: &str, role: Option<&str>) -> Self {
        self.cell = Some(HiveCellMembership {
            id: id.to_string(),
...
mod tests {
    ...
    #[test]
    fn test_hive_extension_creation() {
        let ext = HiveExtension::new()
            .with_source(HiveSource::new("Alpha-2", "object_tracker", "1.3.0"))
            ...
            .with_status(HiveStatus::new(OperationalStatus::Active, 0.91));
    ...
    #[test]
    fn test_hive_extension_to_xml() {
        let ext = HiveExtension::new()
            .with_source(HiveSource::new("Platform-1", "sensor", "1.0.0"))
    ...
    #[test]
    fn test_hive_hierarchy() {
        let hier = HiveHierarchy::new()
    ...
    #[test]
    fn test_hive_capability() {
        let cap = HiveCapability::new("OBJECT_TRACKING", "1.3.0", 0.94, OperationalStatus::Active);
    ...
    #[test]
    fn test_hive_status_readiness_clamped() {
        let status = HiveStatus::new(OperationalStatus::Active, 1.5);
        assert_eq!(status.readiness, 1.0);

        let status2 = HiveStatus::new(OperationalStatus::Degraded, -0.5);
        assert_eq!(status2.readiness, 0.0);
    }

    #[test]
    fn test_hive_extension_with_attributes() {
        let ext = HiveExtension::new()
    ...
    #[test]
    fn test_hive_handoff() {
        let handoff =
            HiveHandoff::new("Alpha-Team", "Bravo-Team", "INITIATED", "boundary_crossing");
    ...
    #[test]
    fn test_hive_classification() {
        let ext = HiveExtension::new().with_classification("UNCLASSIFIED", Some("FOUO"));
