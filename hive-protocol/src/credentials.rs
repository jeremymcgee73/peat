//! # HIVE Credentials - Backend-Agnostic Credential Management
//!
//! Provides unified credential handling for HIVE backends, abstracting
//! away backend-specific naming (e.g., Ditto's app_id/shared_key).
//!
//! ## Environment Variables
//!
//! Primary (recommended):
//! - `HIVE_APP_ID` - Application identifier
//! - `HIVE_SECRET_KEY` - Shared secret key (base64 encoded)
//! - `HIVE_OFFLINE_TOKEN` - Offline license token (for Ditto backend)
//!
...
//!
//! // Load from environment (tries HIVE_*, falls back to DITTO_*)
//! let creds = HiveCredentials::from_env()?;
//!
//! // Access credentials
...
/// HIVE credentials for backend authentication
///
...
impl HiveCredentials {
    ...
    ///
    /// Tries HIVE_* variables first, falls back to DITTO_* for backwards compatibility.
    ///
    /// # Returns
    ///
    /// `Ok(HiveCredentials)` if app_id is found, error otherwise
    ///
    /// # Environment Variables
    ///
    /// | Primary | Legacy Fallback |
    /// |---------|-----------------|
    /// | HIVE_APP_ID | DITTO_APP_ID |
    /// | HIVE_SECRET_KEY | DITTO_SHARED_KEY |
    /// | HIVE_OFFLINE_TOKEN | DITTO_OFFLINE_TOKEN |
    pub fn from_env() -> Result<Self, CredentialsError> {
        let app_id = Self::get_env_with_fallback("HIVE_APP_ID", "DITTO_APP_ID")
    ...
    pub fn is_configured() -> bool {
        Self::get_env_with_fallback("HIVE_APP_ID", "DITTO_APP_ID").is_some()
    }
...
pub enum CredentialsError {
    /// HIVE_APP_ID (or DITTO_APP_ID) not set
    #[error("HIVE_APP_ID not set (also checked DITTO_APP_ID for backwards compatibility)")]
    MissingAppId,

    /// HIVE_APP_ID is empty
    #[error("HIVE_APP_ID cannot be empty")]
    EmptyAppId,

    /// HIVE_SECRET_KEY (or DITTO_SHARED_KEY) not set
    #[error("HIVE_SECRET_KEY not set (also checked DITTO_SHARED_KEY for backwards compatibility)")]
    MissingSecretKey,

    /// HIVE_OFFLINE_TOKEN (or DITTO_OFFLINE_TOKEN) not set
    #[error(
        "HIVE_OFFLINE_TOKEN not set (also checked DITTO_OFFLINE_TOKEN for backwards compatibility)"
    )]
    MissingOfflineToken,
}
...
mod tests {
    ...
    fn test_credentials_without_optional() {
        let creds = HiveCredentials::new("test-app".to_string(), None, None);
    ...
    fn test_require_secret_key_present() {
        let creds = HiveCredentials::new("test-app".to_string(), Some("secret".to_string()), None);
    ...
    fn test_get_env_with_fallback_primary() {
        ...
        let result = HiveCredentials::get_env_with_fallback("NONEXISTENT_VAR", "ALSO_NONEXISTENT");
