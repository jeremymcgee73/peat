# Baseline Testing Requirements for HIVE Protocol Evaluation
...
To properly evaluate HIVE Protocol's architectural benefits, we need structured baseline comparisons across three dimensions:

1. **CAP Full Replication** vs **CAP Differential Filtering** (capability-based optimization)
2. **HIVE Protocol** vs **Ditto Baseline** (CRDT without capability model)
...
- **Ontology separation:** Schema (hive-schema) independent from protocol (hive-protocol)
...
**Tasks:**
1. Create `hive-protocol/examples/traditional_baseline.rs`
...
5. Clear ROI demonstration for HIVE Protocol vs Traditional IoT
...
### Hypothesis: HIVE Protocol Value Proposition
...
To properly evaluate HIVE Protocol's value proposition, we need to compare against **traditional IoT event-driven architectures**, not just CRDT variants. The three-way comparison will demonstrate:
...
3. **Combined Advantage:** HIVE Protocol vs Traditional IoT baseline
