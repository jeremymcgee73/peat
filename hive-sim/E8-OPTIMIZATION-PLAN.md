...
preflight_checks() {
    ...
    if ! docker image inspect hive-sim-node:latest >/dev/null 2>&1; then
        echo "❌ Docker image hive-sim-node:latest not found"
        errors=$((errors + 1))
    fi
...
e8-performance-tests-quick:
	...
	@cd hive-sim && SKIP_IMAGE_CHECK=1 ./run-e8-performance-suite.sh
...
# Add to Makefile
e8-test-quick:
	@cd hive-sim && ./run-e8-performance-suite.sh --filter "100mbps,10mbps" --modes "mode1"
	# Runs only: 3 architectures × 2 bandwidths × 1 mode = 6 tests (~4 minutes)

e8-test-256kbps-only:
	@cd hive-sim && ./run-e8-performance-suite.sh --filter "256kbps"
...
## Prerequisites
- Docker with hive-sim-node:latest image
